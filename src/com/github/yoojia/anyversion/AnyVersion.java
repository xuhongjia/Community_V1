...
public class AnyVersion {
	...
	private AnyVersion(Application context, VersionParser parser) {
		...
		this.mainHandler = new Handler(Looper.getMainLooper()) {
			...
			public void handleMessage(Message msg) {
				NotifyStyle style = NotifyStyle.getStyle(msg.arg1);
				Version remoteVersion = (Version) msg.obj;
				switch (style) {
				case Callback:
					callback.onVersion(remoteVersion);
					break;
				case Broadcast:
					Broadcasts.send(AnyVersion.this.context, remoteVersion);
					break;
				case Dialog:
					new VersionDialog(AnyVersion.this.context, remoteVersion,
							AnyVersion.this.downloads).show();
					break;
				}
			}
		};
		...
		currentVersion = new Version(versionName, null, null, versionCode, 0);
	}
	...
	public static void init(Application context, VersionParser parser) {
		...
		try {
			...
			if (ANY_VERSION != null) {
				Log.e(TAG,
						"Duplicate init AnyVersion ! This VersionParser  will be discard !");
				Log.e(TAG,
						"AnyVersion recommend init on YOUR-Application.onCreate(...) .");
				return;
			}
		} finally {
			LOCK.unlock();
		...
		final Callback core = new Callback() {
			...
			public void onVersion(Version remoteVersion) {
				...
				if (currentVersion.code >= remoteVersion.code)
					return;
				Message msg = Message.obtain(ANY_VERSION.mainHandler, 0,
						style.value(), 0, remoteVersion);
				msg.sendToTarget();
			}
		};
	...
	public void submitDownload(Version version) {
